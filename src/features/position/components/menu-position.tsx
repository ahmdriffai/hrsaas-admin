...
import { ChevronDown, Download, Monitor, RotateCcw, Search, X } from "lucide-react";
import Link from "next/link";
import { useRouter, useSearchParams } from "next/navigation";
import { useState } from "react";
import { FormPosition } from "./form-position";

export default function MenuPosition(): React.ReactNode {
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <span onClick={(e) => e.stopPropagation()} className="flex gap-2">
            <FormPosition />
            <Link href="/companies/positions/visual">
              <Button variant="outline" size="sm" prefixIcon={<Monitor size={16} />}>
                Visual
              </Button>
            </Link>
